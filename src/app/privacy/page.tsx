    ...
    <div className="wrap" style={{ maxWidth: '720px' }}>
      ...
      <div style={SECTION}>
        ...
        <p style={P}>
          ...
          「マイコレクション」で登録した所持カードの情報・買値は、お使いの端末のブラウザ内（localStorage）にのみ保存され、
          当サイトのサーバーには送信・保存されません。
        </p>
        <p style={P}>
          ただし、マイコレクションの「みんなの中での位置」で利用者ご自身が登録ボタンを押した場合に限り、
          <strong>評価額の合計・種類数・枚数（数値のみ）</strong>をサーバーに保存します。
          どのカードを所持しているかは送信しません。保存した数値は順位の算出にのみ使用し、
          他の利用者に個々の金額が表示されることはありません。同じ画面の「登録を解除する」でいつでも削除できます。
          「みんなの予想」に投稿した予想・コメント・表示名も同様にサーバーに保存され、こちらは他の利用者にも表示されます。
        </p>
        <p style={P}>
          また、カード詳細ページの閲覧数（「みんなの注目ランキング」）を集計するため、<strong>どのカードが何日に開かれたか</strong>をサーバーに記録します。
          同じ人を二重に数えないための識別子は、その日限りのランダム化した値（IPアドレスをその日の日付とともにハッシュ化したもの）で、
          IPアドレスそのものや端末を識別する情報は保存せず、日をまたいだ追跡・個人の特定には利用できません。記録は31日で削除されます。
        </p>
      </div>
