...
6. 根拠文（reason）は**このカード固有の情報**で書く。定型文を避けるため以下を厳守する:
   - ⚠ **金額・件数・倍率などの数値を本文に書かないこと**（2026-08-30）。
     価格や出品数は毎日更新されるが、この文章は生成した日のまま残る。同じ画面に
     「出品中 201件」と「本文: 出品件数130件」、「最安出品 ¥15,000」と「本文: ¥13,500」が
     並ぶ事故が実際に起きた。**数字は画面が共通データから出す**ので、本文は数字を持たない。
     「¥」「◯円」「◯件」「◯倍」「◯%」は書かない。
   - 代わりに**固有名詞と、変化しない性質**で固有性を出す（例: イラストレーター名
     「${collector.illustrator}」、ポケモン名/キャラ、収録弾名、絶版/品薄の別、
     描き下ろしか流用か、構築で使われるカードかどうか）。
   - 量に触れる時は程度を言葉で書く（「PSA10が素体より大きく高い」「出品が減っている」
     「取引が少ない」など）。読み手は隣に出ている実数と突き合わせて読める。
...
- **同じ内容を言い換えて重ねない。**
  ...
  - 良い例:「PSA10は素体よりかなり高く取引されています。自分で鑑定に出すつもりなら、いまの素体価格は狙い目です」
    （倍率の実数は画面が別に出すので、本文では程度だけを言う）
...
## 出力ルール
1. 各項目は1〜2文。このカード固有の材料に必ず触れる。
   ⚠ **金額・件数・倍率などの数値は書かない**（2026-08-30）。価格や出品数は毎日更新されるのに
   この文章は生成日のまま残るため、同じ画面で数字が食い違う（実際に「出品中201件」と
   「本文130件」、「最安¥15,000」と「本文¥13,500」が並んだ）。**数字は画面が共通データから出す**。
   量に触れる時は「PSA10が素体より大きく高い」「出品が減っている」のように程度を言葉で書く。
