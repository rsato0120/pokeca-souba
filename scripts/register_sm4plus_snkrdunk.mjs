// SM4+ の snkrdunk apparel ID を登録する。
// ⚠ 書き込む前に /v1/apparels/{id} の localizedName に "[SM4+ {番号}/114]" が入っているかを
...
const MAP = {
  119: 91323, 120: 91325, 118: 91324,
}

const data = JSON.parse(fs.readFileSync(DATA, 'utf-8'))
const cards = data.cards.filter((c) => c.box_id === 'gx_battle_boost')
...
for (const card of cards) {
  ...
  const want = `[SM4+ ${String(no).padStart(3, '0')}/114]`
