...
for (const set of sets) {
  if (set.id !== 'promo') assert.match(set.release_date, /^\d{4}-\d{2}-\d{2}$/)
  assert.ok(products.some(p => p.set_id === set.id))
  if (set.id !== 'promo') assert.equal(products.filter(p => p.set_id === set.id && p.kind === 'box').length, 1)
}
assert.ok(products.every(p => sets.some(s => s.id === p.set_id)))
